...
4. If one transport can reach multiple WordPress sites, resolve the exact intended site identity before site-scoped reads or writes, keep that target explicit through the operation, and re-check it when context is ambiguous or target selection changes.
